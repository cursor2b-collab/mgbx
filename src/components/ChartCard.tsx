...
export function ChartCard() {
  ...
  return (
    <div className="relative w-[300px] h-[300px] rounded-[32px] p-[1.6px]" 
         style={{
           background: 'linear-gradient(to top, rgba(255, 255, 255), rgba(255, 255, 255, 0.1))',
           boxShadow: '0 0px 80px -10px rgba(255, 255, 255, 0.15)'
         }}>
      {/* Glow effect */}
      <div className="absolute bottom-[-40px] left-1/2 -translate-x-1/2 w-20 h-20 bg-[#777777] -z-10 blur-[70px]" />
      
      {/* Inner card */}
      <div className="w-full h-full rounded-[32px] flex flex-col overflow-hidden"
           style={{
        ...
        <div className="flex items-center justify-between p-5">
          {periods.map((period) => (
            <label key={period} className="relative flex items-center justify-center w-10 rounded-xl text-[#a7a7a7] text-[8px] font-semibold cursor-pointer hover:text-white transition-colors">
              <input
                type="radio"
                name="period"
                ...
                className="hidden"
              />
              <span className={`w-full py-1.5 flex items-center justify-center rounded-xl z-[1] transition-all ${
              ...
              style={selectedPeriod === period ? {
                ...
                {selectedPeriod === period && (
                  <span className="absolute inset-[1px] bg-[#212121] rounded-xl -z-[1]" />
                )}
                {period}
              </span>
            </label>
        ...
        <div className="flex flex-col px-5">
          <p className="bg-gradient-to-tl from-[#92400e] via-[#f9d86d] to-[#a6a6a6] bg-clip-text text-transparent">
            BTC 70K
          </p>
          <p className="bg-gradient-to-r from-[#8e1414] via-white to-white bg-clip-text text-transparent">
            -2.92%
          </p>
        </div>
        ...
        <div className="relative w-full h-full group">
          {/* Week chart */}
          <i className="absolute inset-0 flex w-full h-full icon-week">
            <svg viewBox="0 0 469 262" fill="none" className="transition-transform duration-500 ease-in-out group-hover:scale-150">
              <path 
          ...
          {/* Month chart */}
          <i className="absolute inset-0 flex w-full h-full icon-month">
            <svg fill="none" viewBox="0 0 472 170" className="transition-transform duration-500 ease-in-out group-hover:scale-150">
              <path 
          ...
          <div className="absolute inset-0 flex justify-between">
            {[...Array(4)].map((_, i) => (
              <span 
                key={i}
                className="w-[1.5px] h-full mx-[18px]"
                style={{
                  background: 'linear-gradient(to top, rgba(255, 255, 255, 0) 0%, rgba(255, 255, 255, 0.025) 50%, rgba(255, 255, 255, 0) 100%)'
                }}
              />
            ))}
          </div>

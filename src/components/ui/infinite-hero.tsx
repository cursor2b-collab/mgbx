...
// SplitText 是 GSAP 的付费插件，如果不可用则使用替代方案
let SplitText: any = null;
try {
  // 尝试动态导入 SplitText（如果已安装）
  SplitText = require("gsap/SplitText").SplitText;
  if (SplitText) {
    gsap.registerPlugin(SplitText);
  }
} catch (e) {
  // SplitText 不可用，将使用替代方案
...
export default function InfiniteHero() {
	...
	useGSAP(
		() => {
			...
			let h1Split: any = null;
			let pSplit: any = null;

			// 如果 SplitText 可用，使用它；否则使用替代方案
			if (SplitText && h1Ref.current && pRef.current) {
				try {
					h1Split = new SplitText(h1Ref.current, { type: "lines" });
					pSplit = new SplitText(pRef.current, { type: "lines" });
				} catch (e) {
					console.log("SplitText initialization failed, using fallback");
				}
			}

			const h1Lines = h1Split?.lines || (h1Ref.current ? [h1Ref.current] : []);
			const pLines = pSplit?.lines || (pRef.current ? [pRef.current] : []);

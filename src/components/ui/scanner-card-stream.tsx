...
const ScannerCardStream = ({
  ...
  useEffect(() => {
    if (cardLineRef.current && cardLineRef.current.parentElement) {
      const containerWidth = cardLineRef.current.parentElement.offsetWidth;
  ...
  const resetPosition = useCallback(() => {
    if (cardLineRef.current) {
        cardStreamState.current.position = cardLineRef.current.parentElement?.offsetWidth || window.innerWidth;
  ...
  // Initialize position on mount - start from a visible position
  useEffect(() => {
    if (cardLineRef.current && cardLineRef.current.parentElement) {
      const containerWidth = cardLineRef.current.parentElement.offsetWidth || window.innerWidth;
      // Start from right edge so cards can flow in from right to left
      cardStreamState.current.position = containerWidth;
      cardStreamState.current.cardLineWidth = (400 + cardGap) * cards.length;
      // Set initial transform immediately
      if (cardLineRef.current) {
        cardLineRef.current.style.transform = `translateX(${containerWidth}px)`;
      }
    }
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent | TouchEvent) => {
      ...
      cardStreamState.current.velocity = Math.abs(deltaX) * 10;
      cardStreamState.current.lastMouseX = clientX;
    };
    const handleMouseUp = () => {
    ...
    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      cardStreamState.current.velocity += e.deltaY * 0.5;
      cardStreamState.current.velocity = Math.max(30, Math.min(500, cardStreamState.current.velocity));
    ...
    // Update card line width and initialize position
    const containerWidth = cardLine.parentElement?.offsetWidth || window.innerWidth;
    cardStreamState.current.cardLineWidth = (400 + cardGap) * cards.length;
    
    // Initialize position if not set
    if (cardStreamState.current.position === 0) {
      cardStreamState.current.position = containerWidth;
    }
    ...
    const animate = (currentTime: number) => {
      ...
      const currentContainerWidth = cardLine.parentElement?.offsetWidth || window.innerWidth;
      if (position < -cardLineWidth) cardStreamState.current.position = currentContainerWidth;
      else if (position > currentContainerWidth) cardStreamState.current.position = -cardLineWidth;
      cardLine.style.transform = `translateX(${cardStreamState.current.position}px)`;
      updateCardEffects();
    ...
    <section className="relative py-12 lg:py-16 bg-black overflow-hidden">
      ...
      {showControls && (
        <div className="absolute top-4 left-4 z-30 flex gap-2">
          <button onClick={toggleAnimation} className="px-4 py-2 bg-white/10 text-white rounded">
            {isPaused ? 'Play' : 'Pause'}
          </button>
          <button onClick={resetPosition} className="px-4 py-2 bg-white/10 text-white rounded">
            Reset
          </button>
          <button onClick={changeDirection} className="px-4 py-2 bg-white/10 text-white rounded">
            Reverse
          </button>
        </div>
      )}
      ...
      {showSpeed && (
        <div className="absolute top-4 right-4 z-30 text-white">
          Speed: {speed}
        </div>
      )}

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative" style={{ height: '280px', minHeight: '280px' }}>
        ...
        <div className="absolute w-full h-[250px] flex items-center overflow-hidden">
          <div ref={cardLineRef} className="flex items-center whitespace-nowrap cursor-grab select-none will-change-transform" style={{ gap: `${cardGap}px` }}>
            {cards.map(card => (
              <div key={card.id} className="card-wrapper relative w-[400px] h-[250px] shrink-0" style={{ '--clip-right': '0%', '--clip-left': '0%' } as React.CSSProperties}>
                <div className="card-normal card absolute top-0 left-0 w-full h-full rounded-[15px] overflow-hidden bg-transparent shadow-[0_15px_40px_rgba(0,0,0,0.4)] z-[2]" style={{ clipPath: 'inset(0 0 0 var(--clip-right, 0%))' }}>
                  <img 
                    src={card.image} 
                    alt="Card" 
                    className="w-full h-full object-contain rounded-[15px] transition-all duration-300 ease-in-out brightness-110 contrast-110 shadow-[inset_0_0_20px_rgba(0,0,0,0.1)] hover:brightness-125 hover:contrast-125"
                    loading="lazy"
                    onError={(e) => {
                    ...
                    onLoad={() => {
                      console.log('Image loaded:', card.image);
                    }}
                  />
                </div>
                <div className="card-ascii card absolute top-0 left-0 w-full h-full rounded-[15px] overflow-hidden bg-transparent z-[1]" style={{ clipPath: 'inset(0 calc(100% - var(--clip-left, 0%)) 0 0)' }}>
                  <pre className="ascii-content absolute top-0 left-0 w-full h-full text-[rgba(220,210,255,0.6)] font-mono text-[11px] leading-[13px] overflow-hidden whitespace-pre m-0 p-0 text-left align-top box-border animate-glitch" style={{ maskImage: 'linear-gradient(to right, rgba(0,0,0,1) 0%, rgba(0,0,0,0.8) 30%, rgba(0,0,0,0.6) 50%, rgba(0,0,0,0.4) 80%, rgba(0,0,0,0.2) 100%)' }}>
                    {card.ascii}
                  </pre>
                </div>
              </div>
